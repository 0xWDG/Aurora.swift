...
extension Aurora {
    ...
    class CrashHandler {
        /// Shared Instance
        public static let shared: CrashHandler = Aurora.load.CrashHandler.init()
        ...
        init() {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                Aurora.load.CrashHandler.shared.registerForSignals()
            }
        }
        ...
        func registerForSignals() {
            if !didRegister {
                NSSetUncaughtExceptionHandler(Aurora.CrashHandler.RecieveException)

                for (signalCode, _) in signalCodes {
                    signal(signalCode, Aurora.CrashHandler.RecieveSignal)
                }
